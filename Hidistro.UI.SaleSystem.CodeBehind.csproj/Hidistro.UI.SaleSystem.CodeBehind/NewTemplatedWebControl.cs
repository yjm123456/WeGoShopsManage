...
namespace Hidistro.UI.SaleSystem.CodeBehind
{
	...
	public abstract class NewTemplatedWebControl : System.Web.UI.WebControls.WebControl, System.Web.UI.INamingContainer
	{
        ...
        protected virtual string SkinPath2
        {
            get
            {
                ...
                if (manager == null)
                {
                    ...
                    if (member != null)
                    {
                        ...
                        {
                            manager = ManagerHelper.GetManagerByTenantID(tenantID);
                            vTheme = manager.tMenu.ToString();
                        }
                        else {
                            //若无绑定强制实现超管商户界面
                            manager = ManagerHelper.GetManagerByTenantID(tid);
                            vTheme = manager.tMenu.ToString();
                        }
                    }
                    else
		...
		{
			string text = System.IO.File.ReadAllText(this.Page.Request.MapPath(this.SkinPath2), System.Text.Encoding.UTF8);
			bool result;

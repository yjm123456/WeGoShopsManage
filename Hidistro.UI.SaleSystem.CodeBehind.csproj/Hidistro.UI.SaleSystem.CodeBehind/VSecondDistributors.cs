...
namespace Hidistro.UI.SaleSystem.CodeBehind
{
	...
	public class VSecondDistributors : VMemberTemplatedWebControl
	{
		...
		protected override void AttachChildControls()
		{
            ...
            string TeamAll = DistributorsBrower.GetMyTeams(userIdDistributors.UserId).Rows.Count.ToString();
            if (!string.IsNullOrEmpty(TeamAll))
            {
                this.litMysubMember.Text = TeamAll;
            }
            else
            {
                this.litMysubMember.Text = "0";

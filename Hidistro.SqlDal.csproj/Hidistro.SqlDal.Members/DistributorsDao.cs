...
namespace Hidistro.SqlDal.Members
{
    public class DistributorsDao
    {
        ...
        public bool CreateDistributor(DistributorsInfo distributor)
        {
            ...
            {
                string text = "INSERT INTO aspnet_Distributors(UserId,StoreName,Logo,BackImage,RequestAccount,GradeId,ReferralUserId,ReferralPath, ReferralOrders,ReferralBlance, ReferralRequestBalance,ReferralStatus,StoreDescription,DistributorGradeId) VALUES(@UserId,@StoreName,@Logo,@BackImage,@RequestAccount,@GradeId,@ReferralUserId,@ReferralPath,@ReferralOrders,@ReferralBlance, @ReferralRequestBalance, @ReferralStatus,@StoreDescription,@DistributorGradeId)";
        ...
        /// <param name="monthPath">是否需要查询月销售额</param>
        /// <returns></returns>
        public decimal GetTeamMoneys(int UserID, DateTime startDate,DateTime endDate)
        {
            decimal Teammoney = 0;
            try
            {
                DataTable dt_UserIds = GetMyTeams(UserID);
                string OneSql = "select oi.ItemsCommission as RebeateMoney,o.OrderId from Hishop_Orders as o left join Hishop_OrderItems as oi on o.OrderId=oi.OrderId where  o.ReferralUserId=" + UserID + " and o.OrderStatus='5' "; //时间判断
                //判断时间筛选
                if (Convert.ToDateTime("2000-01-01") != startDate)
                {
                    if (Convert.ToDateTime("2000-01-01") != endDate)
                    {
                        //管理端筛选月份
                        OneSql += " and o.PayDate between '" + startDate.ToString("yyyy-MM-dd") + "' and '" + endDate.ToString("yyyy-MM-dd") + "'";
                    }
                    else
                    {
                        //客户端月团队奖
                        OneSql += " and o.PayDate between '" + startDate.ToString("yyyy-MM-dd") + "' and GETDATE()";
                    }

                }
                DataTable dt_ThisSaleFeeTotal = Query(OneSql).Tables[0];
                for (int j = 0; j < dt_ThisSaleFeeTotal.Rows.Count; j++)
                {
                    //计算本级
                    Teammoney += Convert.ToDecimal(dt_ThisSaleFeeTotal.Rows[j]["RebeateMoney"].ToString());
                }

                for (int i = 0; i < dt_UserIds.Rows.Count; i++)
                {
                    string ParentID = "";
                    //SQL语句
                    string sql = @" select oi.ItemsCommission as itemsCommission,oi.SecondItemsCommission as secondItemsCommission,oi.ThirdItemsCommission as thirdItemsCommission,o.ReferralPath ,o.OrderId from Hishop_Orders as o left join Hishop_OrderItems as oi on o.OrderId=oi.OrderId where  o.ReferralUserId=" + dt_UserIds.Rows[i]["userid"].ToString() + " and o.OrderStatus='5' ";
                    //时间判断
                    if (Convert.ToDateTime("2000-01-01") != startDate)
                    {
                        if (Convert.ToDateTime("2000-01-01") != endDate)
                        {
                            //管理端筛选月份
                            sql += " and o.PayDate between '" + startDate.ToString("yyyy-MM-dd") + "' and '" +endDate.ToString("yyyy-MM-dd") + "'";
                        }
                        else {
                            //客户端月团队奖
                            sql += " and o.PayDate between '" + startDate.ToString("yyyy-MM-dd") + "' and GETDATE()";
                        }

                    }
                    DataTable dt_SaleFeeTotal = Query(sql).Tables[0];
                    //统计销售额
                    for (int j = 0; j < dt_SaleFeeTotal.Rows.Count; j++)
                    {
                        //计算下级
                        Teammoney += Convert.ToDecimal(dt_SaleFeeTotal.Rows[j]["itemsCommission"].ToString());
                        string aaa = dt_SaleFeeTotal.Rows[j]["ReferralPath"].ToString();// 325|2216
                        if (!string.IsNullOrEmpty(aaa))
                        {

                            if (aaa.Contains("|"))
                            {
                                ParentID = aaa.Split('|')[1];
                                //dt_UserIds.Rows[i]["userid"].ToString()=187

                                if (UserID.ToString() == ParentID)
                                {
                                    Teammoney += Convert.ToDecimal(dt_SaleFeeTotal.Rows[j]["secondItemsCommission"].ToString());
                                }
                                else
                                {
                                    Teammoney += Convert.ToDecimal(dt_SaleFeeTotal.Rows[j]["secondItemsCommission"].ToString());
                                    Teammoney += Convert.ToDecimal(dt_SaleFeeTotal.Rows[j]["thirdItemsCommission"].ToString());
                                    continue;
                                }
                            }
                            else if (UserID == Convert.ToInt32(aaa))
                            {
                                Teammoney += Convert.ToDecimal(dt_SaleFeeTotal.Rows[j]["secondItemsCommission"].ToString());
                            }
                            else
        ...
        public System.Data.DataTable GetMyTeams(int UserID)
        {
            ...
            sql_user.Append(" WITH COMMENT_CTE(userid,referraluserid,username,tLevel)AS( ");
            sql_user.Append(" SELECT userid,referraluserid,username,0 AS tLevel FROM aspnet_Members WHERE referraluserid = " + UserID + " ");
            sql_user.Append(" UNION ALL ");
            sql_user.Append(" SELECT c.userid,c.referraluserid,c.username,ce.tLevel + 1 FROM aspnet_Members AS c  ");

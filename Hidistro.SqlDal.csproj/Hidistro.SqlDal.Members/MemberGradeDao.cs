...
namespace Hidistro.SqlDal.Members
{
	public class MemberGradeDao
	{
		...
		public IList<MemberGradeInfo> GetMemberGrades(string TenantID,string GradeIds = "")
		{
            string text = " SELECT * FROM aspnet_MemberGrades where TenantID=@TenantID";
			if (!string.IsNullOrEmpty(GradeIds))
			{
				text = text + " and GradeId in(" + GradeIds + ");";
			}
		...
		{
			System.Data.Common.DbCommand sqlStringCommand = this.database.GetSqlStringCommand("SELECT COUNT(GradeId) as Count FROM aspnet_MemberGrades WHERE ((TranVol=@TranVol and TranVol is not null ) or (TranTimes=@TranTimes and TranTimes is not null)) AND GradeId<>@GradeId AND TenantID=@TenantID;");
			...
			this.database.AddInParameter(sqlStringCommand, "GradeId", System.Data.DbType.Int32, memberGrade.GradeId);
            this.database.AddInParameter(sqlStringCommand, "TenantID", System.Data.DbType.String, memberGrade.TenantID);
            return (int)this.database.ExecuteScalar(sqlStringCommand) > 0;
		}
		...
		public bool CreateMemberGrade(MemberGradeInfo memberGrade)
		{
			...
			text += " INSERT INTO aspnet_MemberGrades ([Name], Description, Points, IsDefault, Discount,TranVol,TranTimes,TenantID) VALUES (@Name, @Description, @Points, @IsDefault, @Discount,@TranVol,@TranTimes,@TenantID)";
			System.Data.Common.DbCommand sqlStringCommand = this.database.GetSqlStringCommand(text);
		...
		public bool IsExist(string name,string TenantID)
		{
			string query = "SELECT COUNT(*) FROM dbo.aspnet_MemberGrades WHERE Name='" + name + "' AND TenantID='" + TenantID + "'";
			System.Data.Common.DbCommand sqlStringCommand = this.database.GetSqlStringCommand(query);
			return Convert.ToInt32(this.database.ExecuteScalar(sqlStringCommand)) > 0;
		}

		public int SetUserGroup(int day,string TenantID)
		{
			string query = "UPDATE dbo.Hishop_UserGroupSet SET ActiveDay=" + day+" WHERE TenantID='"+TenantID+"'";
			System.Data.Common.DbCommand sqlStringCommand = this.database.GetSqlStringCommand(query);
			return this.database.ExecuteNonQuery(sqlStringCommand);
		}

		public int SelectUserGroupSet(string TenantID)
		{
			string query = "SELECT ActiveDay FROM  dbo.Hishop_UserGroupSet WHERE TenantID='" + TenantID + "'";
			System.Data.Common.DbCommand sqlStringCommand = this.database.GetSqlStringCommand(query);
			return Convert.ToInt32(this.database.ExecuteScalar(sqlStringCommand));
		}
	}

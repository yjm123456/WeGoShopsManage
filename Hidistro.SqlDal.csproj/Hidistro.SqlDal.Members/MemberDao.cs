...
namespace Hidistro.SqlDal.Members
{
    public class MemberDao
	{
        ...
        {
            string TenantID = SqlFind.FindTenantID();
            string commandText = string.Format("select top 1   isnull(ActiveDay,1)  from Hishop_UserGroupSet where TenantID='"+TenantID+"'", new object[0]);
			int result = 1;
			try
			{
		...
		public DbQueryResult GetMembers(MemberQuery query, bool isNotBindUserName = false)
		{
            ...
            if (!string.IsNullOrEmpty(query.TenantID))
            {
                ...
                stringBuilder.AppendFormat("TenantID = '{0}'", query.TenantID);
            }
		...
		public System.Data.DataTable GetMembersByUserId(int referralUserId, int pageIndex, int pageSize, out int total, string sort, string order)
		{
			...
			StringBuilder stringBuilder = new StringBuilder();
			stringBuilder.Append(" WITH COMMENT_CTE(RowIndex,userid,username,GradeId,GradeName,UserHead,CreateDate,ReferralUserID,OrderDate)AS(");
			stringBuilder.AppendFormat(" SELECT ( select ROW_NUMBER()  OVER ");
			stringBuilder.AppendFormat(" (ORDER BY USERID DESC)) as [RowIndex], userid,username, GradeId, case when GradeId= '3' then '推客' when GradeId = '7' then '推客' when GradeId= '12' then '银牌合伙人'  when GradeId= '13' then '金牌合伙人'end as GradeName ,UserHead,CreateDate,ReferralUserID, LastOrderDate as OrderDate FROM aspnet_Members", new object[0]);
			stringBuilder.AppendFormat(" as m WHERE referraluserid ='"+ referralUserId + "' union all", new object[0]);
			stringBuilder.AppendFormat(" SELECT ( select ROW_NUMBER()  OVER ", new object[0]);
			stringBuilder.AppendFormat(" (ORDER BY c.createDate desc)) as [RowIndex],c.userid,c.username,c.GradeId , case  when c.GradeId= '3' then '推客' when c.GradeId = '7' then '推客' when c.GradeId= '12' then '银牌合伙人'  when c.GradeId= '13' then '金牌合伙人'end as GradeName ,c.UserHead,c.CreateDate,c.ReferralUserID, LastOrderDate as OrderDate  FROM aspnet_Members AS c  ", new object[0]);
			stringBuilder.AppendFormat(" INNER JOIN COMMENT_CTE AS ce ", new object[0]);
			stringBuilder.AppendFormat(" ON c.referraluserid = ce.userid", new object[0]);
			stringBuilder.AppendFormat(" )SELECT CT.RowIndex,CT.userid,CT.username,CT.GradeName,CT.UserHead,CT.CreateDate,CT.ReferralUserID,d.StoreName,CT.OrderDate FROM COMMENT_CTE AS CT", new object[0]);
			stringBuilder.AppendFormat(" left join  aspnet_Distributors  d ", new object[0]);
			stringBuilder.AppendFormat(" on CT.userid=d.userid   ");
            if (!string.IsNullOrEmpty(order))
            {
                stringBuilder.AppendFormat(" ORDER BY CT.CreateDate "+ order);
            }
            else
            {
                stringBuilder.AppendFormat(" ORDER BY CT.CreateDate desc");
            }
		...
		public IList<int> GetSleepGroupMemberList()
        {
            ...
            IList<int> list = new List<int>();
			int activeDay = this.GetActiveDay();
			string query = string.Format("SELECT UserId FROM aspnet_Members  WHERE TenantID='"+TenantID+"' and PayOrderDate is null or PayOrderDate <'" + DateTime.Now.AddDays((double)(-(double)activeDay)).ToString("yyyy-MM-dd HH:mm:ss") + "' ", new object[0]);
			System.Data.Common.DbCommand sqlStringCommand = this.database.GetSqlStringCommand(query);
			using (System.Data.IDataReader dataReader = this.database.ExecuteReader(sqlStringCommand))
			{
				System.Data.DataTable dataTable = DataHelper.ConverDataReaderToDataTable(dataReader);
		...
		public IList<int> GetStoreNameMemberList(string StoreName,string TenantID)
		{
			IList<int> list = new List<int>();
		...
		public IList<int> GetAllMemberList(string TenantID)
		{
			IList<int> list = new List<int>();
			string query = "SELECT UserId FROM dbo.aspnet_Members where Status=1 and TenantID=@TenantID";
			System.Data.Common.DbCommand sqlStringCommand = this.database.GetSqlStringCommand(query);

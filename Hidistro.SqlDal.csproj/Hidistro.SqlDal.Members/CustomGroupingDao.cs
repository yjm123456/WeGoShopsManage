...
namespace Hidistro.SqlDal.Members
{
	public class CustomGroupingDao
	{
		...
		public System.Data.DataTable GetCustomGroupingTable(string TenantID)
		{
			System.Data.DataTable result = null;
			System.Data.Common.DbCommand sqlStringCommand = this.database.GetSqlStringCommand("SELECT *  FROM  Vshop_CustomGrouping where TenantID=@TenantID Order By UpdateTime desc");
		...
		public IList<CustomGroupingInfo> GetCustomGroupingList(string TenantID)
		{
			string query = "SELECT * FROM Vshop_CustomGrouping where TenantID=@TenantID Order By UpdateTime desc";
		...
		public IList<CustomGroupingInfo> GetCustomGroupingList(string TenantID,string customGroupIds = "")
		{
			string text = "SELECT * FROM Vshop_CustomGrouping  where TenantID=@TenantID";
			if (!string.IsNullOrEmpty(customGroupIds))
			{
				text = text + " and id in(" + customGroupIds + ");";
			}
		...
		public int GetGroupIdByGroupName(string TenantID,string groupName, int groupId)
		{
			int result = 0;
			...
			System.Data.Common.DbCommand sqlStringCommand = this.database.GetSqlStringCommand("select top 1 Id from Vshop_CustomGrouping where TenantID=@TenantID and GroupName=@GroupName " + str);
		...
		public string AddCustomGrouping(CustomGroupingInfo customGroupingInfo)
		{
			...
			if (this.GetGroupIdByGroupName(customGroupingInfo.TenantID, customGroupingInfo.GroupName, 0) > 0)
			{
				result = "分组名称已存在";
			}
			else
			{
				System.Data.Common.DbCommand sqlStringCommand = this.database.GetSqlStringCommand("INSERT INTO Vshop_CustomGrouping(GroupName,Memo,TenantID)VALUES(@GroupName,@Memo,@TenantID);select @@identity;");
                ...
                this.database.AddInParameter(sqlStringCommand, "TenantID", System.Data.DbType.String, customGroupingInfo.TenantID);
		...
		{
			string result = string.Empty;
			if (this.GetGroupIdByGroupName(customGroupingInfo.TenantID,customGroupingInfo.GroupName, customGroupingInfo.Id) > 0)
			{
				result = "分组名称已存在";
			}
			else

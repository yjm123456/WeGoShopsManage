...
namespace Hidistro.SqlDal.Orders
{
    public class OrderDao
    {
        ...
        public DbQueryResult GetUserOrderByPage(int userId, OrderQuery query)
        {
            ...
            text3 = text3 + " SELECT ID, OrderId, ThumbnailsUrl, ItemDescription, SKUContent, SKU,OrderItemsStatus, ProductId,Quantity,ReturnMoney,SkuID,ItemAdjustedPrice,Type,PointNumber,LimitedTimeDiscountId,DiscountAverage FROM Hishop_OrderItems WHERE OrderId IN  (" + text2 + ")   ";
        ...
        public System.Data.DataSet GetOrdersByOrderIDList(string orderIds)
        {
            ...
            string text = "  HO.OrderId,ShipTo,RegionId,ExpressCompanyName,ExpressCompanyAbb,HOA.wayBill as ShipOrderNumber,Remark,OrderStatus,ShippingRegion,Address";
            query = string.Format(string.Concat(new string[]
            {
                "with v as (SELECT ",
                text,
                ", row_number() over (partition by ShipTo+CONVERT(VARCHAR(11), RegionId)+[Address]+CellPhone order by  RegionId desc) as rownumber from Hishop_Orders as HO left join Hishop_OrderAlreadySend as HOA on HO.OrderId=HOA.OrderID where  HO.OrderId in ({0})) select ",
                text=" OrderId,ShipTo,RegionId,ExpressCompanyName,ExpressCompanyAbb,ShipOrderNumber,Remark,OrderStatus,ShippingRegion,Address",
        ...
        public bool CreatOrder(OrderInfo orderInfo, System.Data.Common.DbTransaction dbTran)
        {
            ...
            if (orderInfo.LineItems.Count > 0)
            {
                ...
                {
                    if (tenantList.Count == 0 || tenantList.Contains(lineItemInfo.TenantID))
                    {
                        if (string.IsNullOrEmpty(lineItemInfo.TenantID))
                        {
                            string Tid= SqlFind.FindTenantID();
                            if (string.IsNullOrEmpty(Tid))
                            {
                                Tid = ConfigurationManager.AppSettings["SuperAdminTID"].ToString();
                            }
                            lineItemInfo.TenantID = Tid;
                        }
                        tenantList.Add(lineItemInfo.TenantID);
                    }
                    else {
                        //存在不同商家的商品，请拆分成单个提交！
                        Globals.Debuglog("存在不同商家的商品，请拆分成单个提交！", "_DebuglogOrder.txt");
                        return false;

                    }
                }

                this.database.AddInParameter(storedProcCommand, "TenantID", System.Data.DbType.String, tenantList[0].ToString());
            }
        ...
        /// <returns></returns>
        public decimal GetRebeateCommission(string OrderID)
        {
            decimal returncommission = 0;
            try
            {
                string ProductID = ""; string returnCode = "";
                string SelectQuery = string.Format(" SELECT ProductID FROM [dbo].[Hishop_OrderItems]where OrderId= '{0}'", OrderID);
                System.Data.Common.DbCommand sqlStringCommand = this.database.GetSqlStringCommand(SelectQuery);
                System.Data.DataTable dataTable = this.database.ExecuteDataSet(sqlStringCommand).Tables[0];
                if (dataTable.Rows.Count > 0)
                {
                    ProductID = Convert.ToString(dataTable.Rows[0]["ProductID"]);
                ...
                if (!string.IsNullOrEmpty(ProductID))
                {
                    try
                    { 
                        string SelectQuery2 = string.Format(" SELECT RebeateCommission FROM [dbo].[vw_Hishop_BrowseProductList] WHERE ProductId= '{0}'", ProductID);
                        System.Data.Common.DbCommand sqlStringCommand2 = this.database.GetSqlStringCommand(SelectQuery2);
                        System.Data.DataTable dataTable2 = this.database.ExecuteDataSet(sqlStringCommand2).Tables[0];
                        if (dataTable2.Rows.Count > 0)
                        {
                            returnCode = Convert.ToString(dataTable2.Rows[0]["RebeateCommission"]);

                        }
                        if (!string.IsNullOrEmpty(returnCode))
                        {
                            returncommission = Convert.ToDecimal(returnCode);
                        }
                    }
                    catch (Exception err)
                    {
                        string ers = err.Message.ToString();
                        Globals.Debuglog("获取详细佣金失败；" + ers, "_DebuglogOrder.txt");
                    }
                }
            }
            catch (Exception err)
            {
                ...
                Globals.Debuglog("获取订单佣金失败；"+errs, "_DebuglogOrder.txt");
            }

		...
		public bool AddOrderLineItems(string orderId, ICollection lineItems, System.Data.Common.DbTransaction dbTran)
		{
			...
			else
			{
				...
				foreach (LineItemInfo lineItemInfo in lineItems)
				{
					...
					stringBuilder.Append("INSERT INTO Hishop_OrderItems(OrderId, SkuId, ProductId, SKU, Quantity, ShipmentQuantity, CostPrice").Append(",ItemListPrice, ItemAdjustedPrice, ItemDescription, ThumbnailsUrl, Weight, SKUContent, PromotionId, PromotionName,OrderItemsStatus,ItemsCommission,SecondItemsCommission,ThirdItemsCommission,ItemsCommissionScale,SecondItemsCommissionScale,ThirdItemsCommissionScale,PointNumber,Type,DiscountAverage,LimitedTimeDiscountId,CommissionDiscount,TenantID) VALUES(@OrderId").Append(",@SkuId").Append(text).Append(",@ProductId").Append(text).Append(",@SKU").Append(text).Append(",@Quantity").Append(text).Append(",@ShipmentQuantity").Append(text).Append(",@CostPrice").Append(text).Append(",@ItemListPrice").Append(text).Append(",@ItemAdjustedPrice").Append(text).Append(",@ItemDescription").Append(text).Append(",@ThumbnailsUrl").Append(text).Append(",@Weight").Append(text).Append(",@SKUContent").Append(text).Append(",@PromotionId").Append(text).Append(",@PromotionName").Append(text).Append(",@OrderItemsStatus").Append(text).Append(",@ItemsCommission").Append(text).Append(",@SecondItemsCommission").Append(text).Append(",@ThirdItemsCommission").Append(text).Append(",@ItemsCommissionScale").Append(text).Append(",@SecondItemsCommissionScale").Append(text).Append(",@ThirdItemsCommissionScale").Append(text).Append(",@PointNumber").Append(text).Append(",@Type").Append(text).Append(",@DiscountAverage").Append(text).Append(",@LimitedTimeDiscountId").Append(text).Append(",@CommissionDiscount").Append(text).Append(",@TenantID").Append(text).Append(");");
                    ...
                    this.database.AddInParameter(sqlStringCommand, "TenantID" + text, System.Data.DbType.String, lineItemInfo.TenantID);
                    num++;
					if (num == 50)
					{

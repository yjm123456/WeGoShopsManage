...
namespace Hidistro.SqlDal.Store
{
	public class PhotoGalleryDao
	{
		...
		public bool AddPhotoCategory(string name,string TenantID)
		{
			System.Data.Common.DbCommand sqlStringCommand = this.database.GetSqlStringCommand("DECLARE @DisplaySequence INT; SELECT @DisplaySequence = ISNULL(MAX(DisplaySequence), 0) + 1 FROM Hishop_PhotoCategories; INSERT Hishop_PhotoCategories (CategoryName, DisplaySequence,TenantID) VALUES (@CategoryName, @DisplaySequence,@TenantID)");
		...
		public int AddPhotoCategory2(string name,string TenantID)
		{
			System.Data.Common.DbCommand sqlStringCommand = this.database.GetSqlStringCommand("DECLARE @DisplaySequence INT; SELECT @DisplaySequence = ISNULL(MAX(DisplaySequence), 0) + 1 FROM Hishop_PhotoCategories; INSERT Hishop_PhotoCategories (CategoryName, DisplaySequence,TenantID) VALUES (@CategoryName, @DisplaySequence,@TenantID);SELECT @@IDENTITY");
			this.database.AddInParameter(sqlStringCommand, "CategoryName", System.Data.DbType.String, name);
            this.database.AddInParameter(sqlStringCommand, "TenantID", System.Data.DbType.String, TenantID);
		...
		public System.Data.DataTable GetPhotoCategories(int type,string TenantID)
		{
			...
			System.Data.Common.DbCommand sqlStringCommand = this.database.GetSqlStringCommand("SELECT *, (SELECT COUNT(PhotoId) FROM Hishop_PhotoGallery WHERE CategoryId = pc.CategoryId and (TenantID='" + TenantID + "' or TenantID is null) ) AS PhotoCounts FROM Hishop_PhotoCategories pc " + str + " and (TenantID='" + TenantID + "' or TenantID ='') ORDER BY DisplaySequence DESC");
		...
		public DbQueryResult GetPhotoList(string keyword, int? categoryId, Pagination page, int type,string TenantID)
		{
			string text = string.Empty;
			if (type != -1)
			{
            ...
            if (!string.IsNullOrEmpty(TenantID))
            {
                ...
                text += string.Format(" (TenantID  IS NULL OR");
                text += string.Format(" TenantID = {0})", "'"+TenantID+"'");
            }
		...
		public bool AddPhote(int categoryId, string photoName, string photoPath, int fileSize,string TenantID)
		{
           
            System.Data.Common.DbCommand sqlStringCommand = this.database.GetSqlStringCommand("INSERT INTO Hishop_PhotoGallery(CategoryId, PhotoName, PhotoPath, FileSize, UploadTime, LastUpdateTime,TenantID) VALUES (@CategoryId, @PhotoName, @PhotoPath, @FileSize, @UploadTime, @LastUpdateTime,@TenantID)");
			this.database.AddInParameter(sqlStringCommand, "CategoryId", System.Data.DbType.Int32, categoryId);
		...
		public int GetDefaultPhotoCount(string TenantID)
		{
			System.Data.Common.DbCommand sqlStringCommand = this.database.GetSqlStringCommand("SELECT count(*) FROM Hishop_PhotoGallery where CategoryId=0 and TypeId=0 and TenantID=@TenantID");
            this.database.AddInParameter(sqlStringCommand, "TenantID", System.Data.DbType.String, TenantID);
            return Convert.ToInt32(this.database.ExecuteScalar(sqlStringCommand));
		}
	}

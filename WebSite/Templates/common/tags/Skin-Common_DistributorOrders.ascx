...
<div class="member-orders-content" >
    ...
    <div class="info">
       ...
       <p class="text-muted update-price">
           <em>数量:<%# Eval("Quantity")%></em>
           佣金:
           <span class="text-color">
               <%#(!(Eval("OrderItemsStatus").ToString().Equals("9")||Eval("OrderItemsStatus").ToString().Equals("10")))?(Eval("IsAdminModify").ToString()=="0"? (decimal.Parse(Eval("ItemsCommission", "{0:F2}"))-decimal.Parse(Eval("ItemAdjustedCommssion","{0:F2}"))).ToString(): decimal.Parse(Eval("ItemsCommission", "{0:F2}")).ToString()):"0" %>元
           </span>
           <%# (Eval("OrderItemsStatus").ToString().Equals("1")&&Eval("Type").ToString().Equals("0")&&!Eval("Type").ToString().Equals("4")) ?
                   "<a onclick=\"UpdatePrice(" + Eval("ItemsCommission", "{0:F2}") + ",'" + Eval("OrderId") + "','"+Eval("ID")+"')\"></a>" 
                   : ""%>
           <%--app端待付款佣金改价模块 /a 标签前加入“改价”即可--%>
       </p>
    </div>
 </div>

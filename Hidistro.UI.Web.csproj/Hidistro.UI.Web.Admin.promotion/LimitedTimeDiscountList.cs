	...
	public class LimitedTimeDiscountList : AdminPage
	{
		...
		public string GetMemberGarde(string applyMembers, string defualtGroup, string customGroup)
        {
			...
			if (!string.IsNullOrEmpty(applyMembers))
			{
				...
				if (applyMembers == "0")
				{
					memberGrades = MemberHelper.GetMemberGrades(member.TenantID);
				}
				else
				{
					memberGrades = MemberHelper.GetMemberGrades(member.TenantID,applyMembers);
				}
			...
			if (!string.IsNullOrEmpty(customGroup))
			{
				...
				if (customGroup == "0")
				{
					list = CustomGroupingHelper.GetCustomGroupingList(TenantID);
				}
				else if (customGroup != "-1")
				{
					list = CustomGroupingHelper.GetCustomGroupingList(TenantID,customGroup);
				}

...
namespace Hidistro.UI.Web.Admin.Fenxiao
{
	public class DistributorDetails : AdminPage
	{
        ...
        protected System.Web.UI.HtmlControls.HtmlGenericControl TeamMoney;
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
            ...
            ////获取团队奖时间
            //string StartDate = Request["StartTime"].ToString();
            //string EndDate = Request["EndTime"].ToString();
            string result = this.Page.Request.QueryString.ToString();
            string StartDate = "";
            string EndDate = "";
            if (result.Contains("&"))
            {
                StartDate = result.ToString().Split('&')[1];
                StartDate = StartDate.ToString().Split('=')[1];
                EndDate = result.ToString().Split('&')[2];
                EndDate = EndDate.ToString().Split('=')[1];
            }
            ...
            DateTime EndTime = Convert.ToDateTime("2000-01-01"); ;
            ...
            string TeamMoneys = Hidistro.SaleSystem.Vshop.DistributorsBrower.GetTeamMoneysBrowser(distributorsQuery.UserId, StartTime, EndTime).ToString("F2");
            this.TeamMoney.InnerText = TeamMoneys;
            this.BindData(distributorsQuery.UserId);
            Response.Write(TeamMoneys);
        }

...
namespace Hidistro.UI.Web.API
{
	public class VshopProcess : System.Web.IHttpHandler, System.Web.SessionState.IRequiresSessionState
	{
        ...
        string TenantID = MemberHelper.ReturnUserTenID();
		...
		public string ordersummit(ShoppingCartInfo cart, System.Web.HttpContext context, string remark, int shippingId, string couponCode, string selectCouponValue, string shippingTypeinfo, bool summittype, string OrderMarking, System.Collections.Generic.IList<ShoppingCartItemInfo> ItemInfo, int PointExchange, int bargainDetialId, out string ActivitiesIds)
		{
			...
			if (orderInfo != null)
			{
				...
				try
				{
					...
					{
						stringBuilder.Append("\"Status\":\"Error\"");
						stringBuilder.AppendFormat(",\"ErrorMsg\":\"提交订单失败，不同商家订单请拆分提交！\"", new object[0]);
					}
					goto IL_110E;
				}
		...
		private void ProcessSubmmitorder(System.Web.HttpContext context)
		{
				...
				else
				{
					...
					if (num5 > 0)
					{
						...
						if (flag3)
						{
							...
							{
								if (MemberHelper.CheckCurrentMemberIsInRange(discountInfo.ApplyMembers, discountInfo.DefualtGroup, discountInfo.CustomGroup, currentMember.UserId,TenantID))
								{
			...
			else
			{
				...
				{
					string submtResult= this.ordersummit(current4, context, array3[num7], shippingId, couponCode, array[num7], array2[num7], true, text4, current4.LineItems, 0, 0, out text3);
					text2 = text2 + text3 + ",";
                    if (submtResult.Contains("Error")==true)
                    {
                        stringBuilder.Append(submtResult);
                        goto IL_550;
                    }
					num7++;
				}

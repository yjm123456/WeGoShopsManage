...
namespace Hidistro.UI.Web.Vshop
{
	public class Default : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
        {
            ...
            var AdminID = Globals.GetCurrentMemberUserId();
            if (AdminID != 0)
            {
                var Member = MemberHelper.GetMember(AdminID);
                if (Member == null)
                {

                    //数据库中未检索到该客户 则使用超管展示界面
                    var  m = ManagerHelper.GetManagerByTenantID(ConfigurationManager.AppSettings["SuperAdminTID"].ToString());
                    this.siteSettings.VTheme = m.tMenu.ToString();
                }
                else
                {
                    ManagerInfo manager = ManagerHelper.GetManagerByTenantID(Member.TenantID);
                    if (manager == null)
                    {
                        //普通游客（未绑定商户ID）则使用超管展示界面
                        manager = ManagerHelper.GetManagerByTenantID(ConfigurationManager.AppSettings["SuperAdminTID"].ToString());
                        this.siteSettings.VTheme = manager.tMenu.ToString();
                    }
                    else
                    {
                        //判断该商户是否设置主界面
                        ...
                        {
                            this.siteSettings.VTheme = t.ToString();
                        }
                        else {
                            //未设置主界面则使用超管展示界面
                            var ms = ManagerHelper.GetManagerByTenantID(ConfigurationManager.AppSettings["SuperAdminTID"].ToString());
                            this.siteSettings.VTheme = ms.tMenu.ToString();
                        }
                    }
                }
            }

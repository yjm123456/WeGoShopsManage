    ...
    public static class OrderHelper
    {
        ...
        public static bool ConfirmPay(OrderInfo order)
        {
            ...
            if (order.CheckAction(OrderActions.SELLER_CONFIRM_PAY))
            {
                ...
                Orders = connection.GetProductID(sql2, order.OrderId);
                if (Orders.Count > 0)
                {
                    for (int j = 0; j < Orders.Count; j++)
                    {
                        ...
                        o.Username = order.ShipTo;
                        o.CellPhone = order.CellPhone;
                        o.ShippingRegion = order.ShippingRegion;
                        o.Remark = order.Remark;
                        o.Address = order.Address;
                        o.SKU = Orders[j].SKU.ToString();
                        o.ProductId = Orders[j].ProductId;
                        o.Quantity = Orders[j].Quantity;
                ...
                selectResult = connection.GetProductCheckResult(sqlCheck, order.OrderId);
                //若查询结果为空则继续
                if (selectResult.Count != 0)
                {
                    //记录日志
                    Globals.Debuglog("该订单号已提交过京东，请勿重复提交！", "_JDDebuglog.txt");
                    return false;
                }
                if (UsingOrder.Count > 0)
                {
                    ...
                    string jdClientConn = ConfigurationManager.AppSettings["JDClientConnect"].ToString();
                    Globals.Debuglog("获取京东对接配置："+jdClientConn, "_JDDebuglog.txt");
                    if (jdClientConn == "false")
                    {
                        res = true;
                    }
                    if (res != true)
                    {
                        Globals.Debuglog("无法修改订单状态，订单提交到京东失败，请重试！", "_JDDebuglog.txt");
                        return false;
                    }
                }
                #endregion
                ...
                if (flag)
                {
                    ...
                    {
                        string catchres = var_13_24F.Message.ToString();
                        //记录日志
                        Globals.Debuglog("发送微信消息失败,原因："+catchres, "_Debuglog.txt");
                    }

...
namespace Hidistro.Messages
{
    public static class Messenger
    {
       ...
       private static string Distinct = "";
        ...
        /// 组合发送订单数据
        /// </summary>
        /// <param name="templateId">微信模板ID</param>
        /// <param name="settings">配置账号以及TOKEN等信息</param>
        ...
        /// <param name="FirstData"></param>
        /// <returns></returns>
        private static TempleteModel GenerateWeixinMessage_GetBuyInformationMsg(string templateId, MemberInfo member, OrderInfo order, string FirstData)
        {
            string firstProductName = (new OrderDao()).GetFirstProductName(order.OrderId);
            TempleteModel templeteModel = new TempleteModel()
            {
            ...
            TempleteModel variable = templeteModel;
            TemplateDataItem templateDataItem = new TemplateDataItem((string.IsNullOrEmpty(FirstData) ? "您好，有一位新分销商申请了店铺" : FirstData), "#173177");
            TemplateDataItem templateDataItem1 = new TemplateDataItem((string.IsNullOrEmpty(firstProductName) ? "未知产品" : firstProductName), "#173177");
            TemplateDataItem templateDataItem2 = new TemplateDataItem(member.UserName, "#173177");
            decimal amount = order.TotalPrice;
            //上下打印日志
            AliOHHelper.log("开始打印获取佣金日志");
            //获取佣金参数
            decimal RebeateMoney = (new OrderDao()).GetRebeateCommission(order.OrderId);
            TemplateDataItem templateDataItem3 = new TemplateDataItem(amount.ToString("F2") + "；预计产生佣金：" + RebeateMoney.ToString("F2"), "#173177");
            DateTime now = DateTime.Now;
            variable.data = new { first = templateDataItem, keyword1 = templateDataItem2, keyword2 = templateDataItem1, keyword3 = templateDataItem3, keyword4 = new TemplateDataItem(now.ToString("yyyy-MM-dd HH:mm:ss"), "#173177") };
            return templeteModel;
        }
        ...
        {
            AliOHHelper.log("获取发送参数： BuyerWXOpenId：" + BuyerWXOpenId + "；SalerWXOpenId：" + SalerWXOpenId);
            List<string> strs = new List<string>();
        ...
        /// <param name="member"></param>
        /// <param name="AliOpneid"></param>
        //public static void SendFuwuMsg_OrderGetBuyInforMation(OrderInfo order, MemberInfo member ,string AliOpneid)
        //{
        //    string str = "OrderGetBuyInforMation";
        //    //获取数据库数据
        ...
        //        SiteSettings masterSettings = SettingsManager.GetMasterSettings(true);
        //        //组合发送数据（重新整改）
        //        //AliTemplateMessage aliTemplateMessage = Messenger.GenerateFuwuMessage_OrderMsg(messageTemplateByDetailType.WeixinTemplateId, masterSettings, order, string.Concat("您好，您的下级", member.UserName, "已完成商品下单"));
        //        //TempleteModel model = new TempleteModel();
        //        TempleteModel templeteModel1 = Messenger.GenerateWeixinMessage_GetBuyInformationMsg(messageTemplateByDetailType.WeixinTemplateId, member, order, string.Format("您好，您的下级", member.UserName, "已完成商品下单"));
        //        Messenger.Send_WeiXin_ToOneUser(member.OpenId, templeteModel1);
        //    }
        //}
        ...
        public static void SendFuwuMsg_OrderPay(OrderInfo order)
        {
            ...
            //        TempleteModel templeteModel2 = Messenger.GenerateWeixinMessage_GetBuyInformationMsg(messageTemplateByDetailType.WeixinTemplateId, member, order, string.Format("您好，您的下级", member.UserName, "已完成商品下单"));
        ...
        public static void SendWeiXinMsg_OrderPay(OrderInfo order)
        {
            ...
            {
                string ParentUserID = "";
                AliOHHelper.log("给所属上级发送通知");
                if (order.ReferralPath.Contains("|"))
                {
                    ParentUserID = order.ReferralPath.ToString().Split('|')[1];
                }
                else
                {
                    ParentUserID = order.ReferralPath;
                }
                string str3 = "OrderGetBuyInforMation";
                //获取数据库数据
                MessageTemplate OrderBuyInformation = MessageTemplateHelper.GetMessageTemplateByDetailType(str3);
                //如果在数据库中查询的到
                if (OrderBuyInformation != null)
                {
                    string openIDByUserId = (new MemberDao()).GetOpenIDByUserId(Convert.ToInt32(ParentUserID));
                    if (!string.IsNullOrEmpty(openIDByUserId))
                    {
                        string Informations = "您好，您有新的付款通知~";
                        strs.Clear();
                        strs.Add(openIDByUserId);
                        MemberInfo member = new MemberInfo();
                        member.UserName = order.Username;
                        TempleteModel templeteModel2 = Messenger.GenerateWeixinMessage_GetBuyInformationMsg(OrderBuyInformation.WeixinTemplateId, member, order, Informations);
                        Messenger.Send_WeiXin_ToListUser(strs, templeteModel2);
                    }
                }
            }
        ...
        {
            AliOHHelper.log("目前Distinct参数："+ Distinct);
            if (refundInfo.OrderId==Distinct)
            {
                return;
            }
            List<string> strs = new List<string>();
            AliOHHelper.log("走微信退款");
            string str;
            string str1;
            ...
            //给所属上级发送通知
            if (!string.IsNullOrEmpty(orderInfo.ReferralPath))
            {
                string openIDByUserId = "";
                string ParentUserID = "";
                AliOHHelper.log("给所属上级发送通知");
                if (orderInfo.ReferralPath.Contains("|"))
                {
                    ParentUserID = orderInfo.ReferralPath.ToString().Split('|')[1];
                }
                else {
                    ParentUserID =orderInfo.ReferralPath;
                }
                openIDByUserId = (new MemberDao()).GetOpenIDByUserId(Convert.ToInt32(ParentUserID));
                if (!string.IsNullOrEmpty(openIDByUserId))
                {
                    SettingsManager.GetMasterSettings(true);
                    TempleteModel templeteModel2 = Messenger.GenerateWeixinMessage_RefundSuccessMsg(messageTemplateByDetailType.WeixinTemplateId, orderInfo, refundInfo, "您的下级" + orderInfo.Username + "退款申请已经发放，敬请关注！");
                    if (templeteModel2 != null)
                    {
                        Messenger.Send_WeiXin_ToMoreUser(str2, openIDByUserId, "", messageTemplateByDetailType, true, templeteModel2);
                    }
                }
            }
            Distinct = refundInfo.OrderId;
        }
